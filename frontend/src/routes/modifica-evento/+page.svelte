<script>
  ...
  // Stato del form - inizializzato vuoto
  let formData = {
    titolo: '',
    descrizione: '',
    data: '',
 ...
 let id
  ...
  // Carica i dati dallo store quando il componente viene montato
  onMount(async () => {
    try {
      isLoading = true;
      
      // Sottoscrizione allo store per ottenere i valori correnti
      const unsubscribe = eventoSelezionato.subscribe(evento => {
        if (evento.titolo && evento.descrizione) {
          formData.titolo = evento.titolo;
          formData.descrizione = evento.descrizione;
          
          // Ora puoi fare la chiamata API con questi dati
          caricaDettagliEvento(evento.titolo, evento.descrizione);
        }
      });
      
      // Funzione per pulire la sottoscrizione
      return () => unsubscribe();
    } catch (err) {
      error = err.message;
      isLoading = false;
  ...
  async function caricaDettagliEvento(titolo, descrizione) {
    try {
      ...
      const dataFormattata = dataObj.toISOString().split('T')[0];
      id = evento._id;
      // Aggiorna il form con tutti i dati dell'evento
      formData = {
        ...formData, // Mantieni titolo e descrizione dallo store
        data: dataFormattata,
        orafine: evento.orafine || '',
        luogo: evento.luogo,
        prezzo: evento.prezzo.toString(),
        tipologia: evento.tipologia || 'concerto',
        tags: evento.tags || [],
        copertina: evento.copertina || '',
  ...
  try {
    ...
    alert(data.message || 'Evento aggiornato con successo');
    goto('/profile');
...
<div class="form-container">
  ...
  {#if isLoading && !formData.titolo}
    <div class="loading-message">Caricamento evento...</div>
  {:else}
    {#if error}
